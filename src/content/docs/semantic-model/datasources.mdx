...
- Universes and query plans are built per datasource, and **every query runs in exactly one datasource**. Strata does not merge results across datasources.
...
What multiple datasources are for is **routing**, not blending. Define the same tables and measures in more than one datasource (a Snowflake warehouse of record and a ClickHouse hot tier, say), and the planner picks the datasource that can satisfy the whole query, preferring by partition, tier, and cost. See [Semantic Routing](/docs/advanced/semantic-routing/). A project can also model unrelated domains in different warehouses, as long as no single query needs both.
...
- [Semantic Routing](/docs/advanced/semantic-routing/) — how the planner chooses a datasource
