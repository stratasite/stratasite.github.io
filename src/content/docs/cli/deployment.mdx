...
### Row-Level Security Is Not Available on Your License

```
Row-level security is not available on your current license plan.
```

The project contains a `security.yml` but the server is on the Free plan. Delete `security.yml` and redeploy, or email [sales@strata.do](mailto:sales@strata.do) to upgrade to Team or Enterprise. See [Row-Level Security](/docs/security/).
