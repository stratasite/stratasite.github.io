...
**Rule you must respect:** a query is executed by exactly one datasource. Strata never blends results across datasources, so everything a query asks for must be servable from a single one. When the same measures and dimensions are defined in more than one datasource, routing decides which one runs the query.
...
When two tables define the same measure (e.g. "Store Sales Price"), the [one-name-one-concept rule](/docs/getting-started/concepts/#core-design-principles) makes them one measure with two sources. The planner first keeps the tables whose dimensions cover the query, then prefers the lower-cost one, so `cost` is how you break ties.

### Partition Constraints

**Where:** In the table's YAML under `partition`.  
**What it means:** This table holds data for a specific range (e.g. "Sale Date ≥ 2024-01-01"). The planner prefers tables whose partition matches the query filters so it can prune data. See [Partitions](/docs/semantic-model/partitions/) for full syntax.
...
- [Partitions](/docs/semantic-model/partitions/) — Time-based constraints
